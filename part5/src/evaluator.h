...
#define OPERATOR(operator) OPERATOR_##operator
...
#define EVAL_CASE(operator,ect0,ecast,ect1,ect2) \
	case ect2: op[0]->attr->return_type = ect0; op[0]->ATTR_VTYPE(ect0) = (CAST_TYPE(ecast) op[1]->ATTR_VTYPE(ect1) OPERATOR(operator) CAST_TYPE(ecast) op[2]->ATTR_VTYPE(ect2)); break;
...
#define EVAL_CASES_ARITHMETIC(operator,ect1) \
	case ect1: switch(type2) { \
		EVAL_CASE(operator,RESOLVE(ect1,INT),RESOLVE(ect1,INT),ect1,INT) \
		EVAL_CASE(operator,RESOLVE(ect1,CHAR),RESOLVE(ect1,CHAR),ect1,CHAR) \
		EVAL_CASE(operator,RESOLVE(ect1,FLOAT),RESOLVE(ect1,FLOAT),ect1,FLOAT) \
...
#define EVAL_CASES_LOGICAL(operator,ect1) \
	case ect1: switch(type2) { \
		EVAL_CASE(operator,INT,RESOLVE(ect1,INT),ect1,INT) \
		EVAL_CASE(operator,INT,RESOLVE(ect1,CHAR),ect1,CHAR) \
		EVAL_CASE(operator,INT,RESOLVE(ect1,FLOAT),ect1,FLOAT) \
		default: error = true; error_type2(node->name, tc_type_str(type1), tc_type_str(type2)); break; \
	} break; \

#define EVALUATE(operator) \
	int type1 = op[1]->attr->return_type; \
	int type2 = op[2]->attr->return_type; \
	switch(type1) { \
		EVAL_CASES_##operator(INT) \
		EVAL_CASES_##operator(CHAR) \
		EVAL_CASES_##operator(FLOAT) \
		default: error = true; error_type2(node->name, tc_type_str(type1), tc_type_str(type2)); break; \
	} \
